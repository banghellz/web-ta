...
class MissingToolsController extends Controller
{
    ...
    public function export(Request $request)
    {
        try {
            ...
            $filename = 'my_missing_tools_' . date('Y-m-d_H-i-s') . '.csv';
            ...
            $callback = function () use ($missingTools) {
                ...
                // CSV Data
                foreach ($missingTools as $missingTool) {
                    $actionDate = '';
                    if ($missingTool->status === 'completed' && $missingTool->reclaimed_at) {
                        $actionDate = $missingTool->reclaimed_at->format('d/m/Y H:i');
                    } elseif ($missingTool->status === 'cancelled' && $missingTool->updated_at) {
                        $actionDate = $missingTool->updated_at->format('d/m/Y H:i');
                    }

                    fputcsv($file, [
                        $missingTool->epc,
                        $missingTool->nama_barang,
                        $missingTool->status_text,
                        $missingTool->reported_at ? $missingTool->reported_at->format('d/m/Y H:i') : '-',
                        $actionDate ?: '-',
                        $missingTool->duration
                    ]);
                }
            ...
            Log::error('Export missing tools error: ' . $e->getMessage());
